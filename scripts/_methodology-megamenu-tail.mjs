// Final pass: handle the two slim-header pages that had a trailing
// blank line before </nav> (which made the prior sweep's exact-string
// anchor miss them). Same outcome — replace flat nav with mega-menu
// nav + inject the three mega-menu panels before </header>.
...
const OLD_NAV_TRAIL = `    <nav class="hidden xl:flex items-center gap-6 h-full mx-6" aria-label="Primary">
...
for (const f of FILES) {
  ...
  if (!before.includes(OLD_NAV_TRAIL)) { nohook++; console.warn('nohook:', f); continue; }
  let after = before.replace(OLD_NAV_TRAIL, NEW_NAV);
